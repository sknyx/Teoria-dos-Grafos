...
void dijkstra(Graph* graph, int ordem, int startnode)
{
    ...
    for(i=0; i<ordem; i++)
    {
        if(i!=startnode)
        {
            if(distance[i] != 999)
            {
                printf("\nDistância até '%c' = %d",graph->head[i]->info,distance[i]);
                printf("\nCaminho de '%c' até '%c': (%c) ", graph->head[startnode]->info, graph->head[i]->info, graph->head[i]->info);
            ...
            {
                printf("\nDistância até '%c' = INFINITA",graph->head[i]->info);
                printf("\nNão existe caminho de '%c' até '%c' :( \n", graph->head[startnode]->info, graph->head[i]->info);
            }           
        }
    }
}
